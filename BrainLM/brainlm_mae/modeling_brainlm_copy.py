...
class BrainLMEmbeddings(ViTMAEEmbeddings):
    ...
    def forward(self, signal_vectors, xyz_vectors, noise):
        ...
        print("_________prints from chatGPT_________")
        print(signal_vectors.shape)
        print(batch, num_voxels, self.timepoint_patching_size)
        print(signal_vectors.numel())  # total number of elements
        ...
        #)  # --> [batch, num_voxels, num_patch_tokens, timepoint_patching_size]
                # --- Embedding Projections ---#
        reshaped_signal_vectors = torch.reshape(
            signal_vectors, (batch, -1, num_patch_tokens, self.timepoint_patching_size)
        )  # --> [batch, num_voxels, num_patch_tokens, timepoint_patching_size]
        signal_projection = self.signal_embedding_projection(reshaped_signal_vectors)

        # Project xyz coordinates into spatial embedding
        print("signal projection shape:", signal_projection.shape)
        print("xyz_vectors shape:", xyz_vectors.shape)
        #xyz_projection = self.xyz_embedding_projection(
        #    xyz_vectors
        #)  # --> [batch, num_voxels, hidden_size]
        xyz_projection = xyz_vectors.unsqueeze(2).repeat(1, 1, num_patch_tokens, 1)
        print("xyz_projection shape:", xyz_projection.shape)
        x = (
        ...
        # Random masking
        #TODO check this out! we don't want random masking(?)
    ...
    # This is not used in modeling_brainLM file
